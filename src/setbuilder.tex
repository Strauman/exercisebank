...
\long\def\getnextchar#1#2\endstr{\gdef\exb@nextchar{#1}\gdef\exb@restchars{#2}}
...
\long\def\@execute@exec#1{%
	\def\continueLoop{\isFalse}\@dinfo{Executing custom command stuff}{#1}%
}
...
\long\def\@delegateFileInfo#1{%
	\expandafter\getnextchar#1\endstr%
	\ifnum\pdfstrcmp{\exb@nextchar}{?}=\z@%
		\@execute@makeset@command\exb@restchars
	\else\ifnum\pdfstrcmp{\exb@nextchar}{:}=\z@%
