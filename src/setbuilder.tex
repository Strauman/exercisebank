...
\gdef\isppMode#1#2{
  ...
  \strif{\macroname{#1}}{\ppMode}\relax#2\fi
}
\gdef\@noneofone#1{}
\gdef\@noneOfTwo#1#2{}
...
\gdef\exbank@def@makeset@command#1#2{%
  ...
  \xdef#1{\unexpanded\expandafter{\ea\keep\csname @\macroname\endcsname}}%
  \ea\gdef\csname @\macroname\endcsname{#2}%
}
\let\exbank@isFirstProblem\isTrue
\exbank@def@makeset@command{\phead}{PHEAD}%
...
%:-
\gdef\exec#1{\if\@spriteMode\isFalse?\unexpanded{#1}\else?\relax\fi}
...
%:-
  ...
  \foreach \exerciseFileInfo in \setlist{
    ...
    %% ppList is 0 if it's not a list
    \gdef\ppList{\emptyList}
    ...
    \if[\firstchar%
      ...
      %that are to be sent to \redef. That is from \exclude or \select
      \ea\redef\exerciseFileInfo\relax
      \def\continueLoop{\isTrue}
    \else\if?\firstchar% if it starts with a ?, then it's a macro to be executed
        % Execute given macro only if not spritemode
        \if\@spriteMode\isFalse%
